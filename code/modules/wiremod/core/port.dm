...
/datum/port/proc/set_datatype(type_to_set)
	...
	if(!handler || !handler.is_compatible(src))
		...
		crash_with("[src] port attempted to be set to an incompatible datatype! (target datatype to set: [type_to_set])")
...
/datum/port/proc/on_value_qdeleting(datum/source)
	...
	if(value == source)
		...
		crash_with("Impossible? [src] should only receive COMSIG_PARENT_QDELETING from an atom currently in the port, not [source].")
