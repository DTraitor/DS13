...
/obj/machinery/computer/rdconsole/Initialize()
	.=..()
	files = new /datum/research(src) //Setup the research data holder. SSresearch should be initialised at this point
	SyncRDevices()
	sync_tech()
	...
	if(linked_destroy)
		if(linked_destroy.loaded_item)
			...
			var/list/destroy_list = list(
				...
				"icon" =				getFlatIcon(linked_destroy.loaded_item),
...
/obj/machinery/computer/rdconsole/ui_static_data(mob/user)
	...
	for(var/tech_id in files.all_technologies)
		...
		for(var/A in Tech.unlocks_designs)
			var/datum/design/temp = SSresearch.design_by_id[A]
...
/obj/machinery/computer/rdconsole/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	...
	switch(action)
		if("eject")
			if(text2num(params["eject_type"]) == PROTOLATHE)
				linked_lathe?.eject_sheet(params["sheet_type"], round(text2num(params["amount"])))

			else if(text2num(params["eject_type"]) == IMPRINTER)
				linked_imprinter?.eject_sheet(params["sheet_type"], round(text2num(params["amount"])))

		if("research_tech")
			if(files.all_technologies[params["tech_id"]])
	...
	for(var/I in files.known_designs)
		...
		if(D.build_type & build_type)
			...
			if(D.category)
				cat = D.category
			var/materials = list()
			var/need_materials = list()
			var/chemicals = list()
			var/need_chemicals = list()
			var/c = 50
			var/t
			for(var/M in D.materials)
				...
				if(t < 1)
					materials += list(capitalize(M) = (D.materials[M]*coeff))
				else
					need_materials += list(capitalize(M) = (D.materials[M]*coeff))
				c = min(t,c)
			...
			if(D.chemicals.len)
				for(var/R in D.chemicals)
					...
					if(t < 1)
						chemicals += CallReagentName(R)
					else
						need_chemicals += CallReagentName(R)
					c = min(t,c)
			...
			designs_list += list(list(
				...
				"can_create" =		c,
				"have_mats" =		materials,
				"needed_mats" =		need_materials,
				"have_chems" =		chemicals,
				"needed_chems" =	need_chemicals
			))
	return designs_list
...
/obj/machinery/computer/rdconsole/proc/SyncRDevices() //Makes sure it is properly sync'ed up with the devices attached to it (if any).
	for(var/obj/machinery/r_n_d/D in range(4, src))
		if(D.linked_console || D.panel_open)
			continue
