...
/datum/component/shell/RegisterWithParent()
	...
	if(!(shell_flags & SHELL_FLAG_CIRCUIT_FIXED))
		RegisterSignal(parent, COMSIG_ATOM_TOOL_ACT(QUALITY_SCREW_DRIVING), .proc/on_screwdriver_act)
		RegisterSignal(parent, COMSIG_ATOM_TOOL_ACT(QUALITY_PULSING), .proc/on_multitool_act)
...
/datum/component/shell/UnregisterFromParent()
	UnregisterSignal(parent, list(
		...
		COMSIG_ATOM_TOOL_ACT(QUALITY_SCREW_DRIVING),
		COMSIG_ATOM_TOOL_ACT(QUALITY_PULSING),
...
/datum/component/shell/proc/on_examine(atom/movable/source, mob/user, list/examine_text)
	...
	var/obj/item/weapon/cell/cell = attached_circuit.cell
	...
	if(shell_flags & SHELL_FLAG_REQUIRE_ANCHOR)
		...
		if(!source.anchored)
			examine_text += SPAN_NOTICE("<b>The integrated circuit is non-functional whilst the shell is unanchored.</b>")
...
/datum/component/shell/proc/on_attack_by(atom/source, obj/item/item, mob/living/attacker)
	...
	if(istype(item, /obj/item/weapon/cell))
	...
	if(attached_circuit)
		...
		if(!attached_circuit.owner_id && istype(item, /obj/item/weapon/card/id))
...
/datum/component/shell/proc/attach_circuit(obj/item/integrated_circuit/circuitboard, mob/living/user)
	...
	if(user && !user.unEquip(circuitboard, parent_atom))
		return
	locked = FALSE
...
/datum/component/shell/proc/is_authorized(mob/user)
	...
	if(attached_circuit?.admin_only)
		if(check_rights(user.client, FALSE, R_VAREDIT))
			return TRUE
		return FALSE
